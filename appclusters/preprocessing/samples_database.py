...
from appclusters.preprocessing.pretrained_embedding import PreTrainedEmbeddings
from appclusters.util.train import get_t2p_word_embedding_type
...
def set_embedded_tokens_length():
    db = SamplesDatabase.get()

    print(get_t2p_word_embedding_type())

    lens = []

    for p in db.filter():
        pattern = PreTrainedEmbeddings.get().get_delimiter_regex_pattern()
        tokens = re.split(pattern, db.read(p, 'description_raw'), flags=re.IGNORECASE)
        embs = PreTrainedEmbeddings.get().tokens_to_indices(tokens)
        if get_t2p_word_embedding_type() == "word2vec":
            db.write(p, description_num_tokens_word2vec=len(embs))
        else:
            db.write(p, description_num_tokens_glove=len(embs))
        lens.append(len(embs))

    db.save()


def embedded_tokens_length_stats():
    db = SamplesDatabase.get()

    print(get_t2p_word_embedding_type())

    lens = [length for length in db.filter(('lang', '==', 'en'),
                                           return_value="description_num_tokens_%s" % get_t2p_word_embedding_type())]
    ...
    print("20+ emb  ", len([l for l in lens if l > 20]))

    lens_covered = len([l for l in lens if l < config.Text2PermissionClassifier.max_description_embeddings])/len(lens)*100
    print("<=%3d     %d%%" % (config.Text2PermissionClassifier.max_description_embeddings, lens_covered))

    y, x = np.histogram(lens, bins=[0, 100, 200, 300, 400, 500, 600, 700, 800])
    y, x = y.tolist(), x.tolist()

    #x_new = [(start+end)/2 for start, end in zip(x, x[1:])]

    print(" ".join(["(%.1f, %d)" % (x1, y1) for x1, y1 in zip(x, y)]))
